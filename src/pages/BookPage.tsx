...
function ConfirmedScreen() {
  ...
  // Support both single-ref (legacy) and multi-ref (multi-slot)
  const refs    = state.confirmationRefs?.length ? state.confirmationRefs : (state.confirmationRef ? [state.confirmationRef] : [])
  const ref     = refs[0] ?? ''
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, borderRadius: 12, padding: '16px 20px', marginBottom: 32, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.22)' }}>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, color: '#22C55E' }}>
              {multi ? `${refs.length} Bookings Confirmed!` : 'Booking Confirmed!'}
            </p>
            <p
